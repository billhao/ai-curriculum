...
The format is frozen: only add content into the existing structure — new rows, people, papers, lineage edges. Never add a section, heading, subsection or table column, and never reorganize; content that fits no existing slot goes in a topic guide instead.
